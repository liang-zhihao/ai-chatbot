...
class MongoDB():
    ...
    def new_user_chatbot(self, record) -> bool:
        ...
        # validation check
        query = { "$and": [ {"user_id": record["user_id"]}, {"chatbot_id": record["chatbot_id"]} ] }
        for user in collection.find(query):
            if user["user_id"] == record["user_id"] and user["chatbot_id"] == record["chatbot_id"]:
                return False
    ...
    def update_chat_history(self, user_id, chatbot_id, message) -> bool:
        ...
        collection = db[self.CHAT_COLLECTION]
        query = { "$and": [ {"user_id": user_id}, {"chatbot_id": chatbot_id} ] }
        return collection.update_one(query, {"$set": {"messages": message}}).acknowledged
    ...
    def delete_user_chatbot(self, user_id, chatbot_id) -> bool:
        db = self.get_databse(self.CHAT_DB)
        return db[self.CHAT_COLLECTION].delete_many({"user_id": user_id, "chatbot_id": chatbot_id}).acknowledged
